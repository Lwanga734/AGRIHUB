...
router.post('/auth/login', async (req, res, next) => {
  try {
    ...
    const { data: row } = await db.from('users').select('*').eq('email', email).maybeSingle();
...
router.get('/auth/me', requireAuth, async (req, res, next) => {
  try {
    const { data: row } = await db.from('users').select('id, name, email, role, phone').eq('id', req.user.sub).maybeSingle();
